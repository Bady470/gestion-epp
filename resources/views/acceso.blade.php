...
<style>
:root{
  --green:#39A935; --green-dark:#2d8a2b; --gold:#FFD700;
...
/* Container */
.container{
  width:100%; max-width:1000px; display:flex; border-radius:28px;
...
/* Left illustration */
.illustration-side{
  flex:1;padding:56px 40px;background:linear-gradient(135deg,#2d8a2b 0%,#39A935 100%);
  color:white;display:flex;flex-direction:column;align-items:center;justify-content:center;position:relative;
}
...
.worker-illustration{width:260px;height:260px;margin-top:28px;position:relative}
.worker-head{width:86px;height:86px;background:#ffcc99;border-radius:50%;margin:0 auto;position:relative;top:10px}
.helmet{width:106px;height:56px;background:var(--gold);border-radius:40px 40px 0 0;margin:0 auto;position:relative;top:-28px}
...
.forgot{margin-bottom:18px;text-align:right}
.forgot a{color:var(--green);text-decoration:none;font-weight:700}
...
@media(max-width:900px){
  ...
  .illustration-side{padding:36px}
    ...
    <section class="form-side" aria-label="Formulario iniciar sesión">
      <div>
        ...
        <nav class="tabs" aria-label="Navegación de acceso">
          <a class="tab active" href="login.html" aria-current="page">Iniciar Sesión</a>
          <a class="tab" href="register.html">Crear Cuenta</a>
        </nav>
        ...
        <form class="form" id="loginForm" novalidate>
          ...
          <div class="socials" role="group" aria-label="Login social">
            <button type="button" class="social" onclick="alert('Login con Google - demo')">📧 Google</button>
            <button type="button" class="social" onclick="alert('Login con Microsoft - demo')">💼 Microsoft</button>
          </div>

          <p class="bottom-note">¿No tienes cuenta? <a href="register.html">Crea una aquí</a></p>
        </form>
      </div>
    </section>
  </main>
